...
- **SVG 图表**：`drawChart()` / `drawDetailChart()` 生成 `<rect>`/`<polyline>`/`<circle>` SVG，**动态 viewBox**——宽高读取容器实际渲染尺寸（宽 `svg.clientWidth`、主图高 `svg.getBoundingClientRect()`，被 CSS clamp 在 240~400px），1:1 渲染不变形，`preserveAspectRatio="none"`；柱宽按 slot（`chartW/barCount`）自适应分档（>60px→34px，>30px→26px，>14px→12px，否则 6px），X 轴标签按像素预算抽样（每标签 ≥52px，末日期强制收尾并避让）。使用 `Date.now()` 后缀防 gradient id 冲突。趋势图柱/点带 `data-day` 属性 + `<title>` 原生 tooltip，点击可跳转到对应日期的统计数据（自动切回月度视图并选中该日）。主图柱状图支持**浏览/下载堆叠**（`opts.stackValues`），峰值日金色高亮。**配色系统**：中性灰面板 + 钢青蓝 accent 的现代仪表风——浅色 bg #f4f4f4/#fbfbfb + accent #315f9f + participation 蓝 #3f74c2，暗色 #212121/#262626 + accent #8ba1c0；浏览=中性蓝灰 #8f98a8（亮）/ #9aa4b2（暗）、下载=暖金褐 #9a6700（亮）/ #c19b5c（暗）、趋势线=蓝灰，全部 CSS 变量控制（dashboard.css / popup.css `:root` + `[data-theme=dark]`），图表颜色由 CSS 变量接管。环形图采用低饱和类别色板（--donut-1~6：钢蓝/绿/靛蓝/玫红/赭石/灰）。域名占比环形图 `drawDonut()`（Top 5 + 其他，段间留缝 GAP_DEG 自适应、点击扇区/图例联动行列表，hover 加粗外扩 brightness(1.06)）。
